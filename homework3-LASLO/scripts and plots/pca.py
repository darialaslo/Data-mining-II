...
def RBFKernelPCA(matrix=None,gamma=1,n_components=2):
    ...
    #making sure we have a dxn matrix (we know we have 300 samples)
    ...
    for i in range(n):
        for j in range(n):
            ...
            K[i,j] = np.exp(-gamma * np.square(dist) )
    ...
    K_centered = np.dot(np.dot((I - (1/n)*U),K),(I - (1/n)*U))
    #3. Compute eigenvalues and eigenvactors
    eigen = computePCA(K_centered)
    eigen_values = eigen[0]
    eigen_vectors = eigen[1]

    #4. sort eigen vectors in decreasing order based on eigen values
    # the compute PCA function already sorts the eigenvectors and eigenvalues 

    #the eigen vectors need rescaling
    eigen_vectors = np.sqrt(1/eigen_values) * eigen_vectors


    #5. Return transformed data
    transformed = np.dot(eigen_vectors[:,:n_components].T, K_centered)
    return transformed
'''
