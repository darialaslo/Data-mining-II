...
def computePCA_SVD(matrix=None):
    ...
    L, D, V = linalg.svd(matrix)

    #gettting and sorting eigenvectors and eigenvalues
    eigenvectors = L
    eigenvalues = np.square(D)/matrix.shape[0]
    ind = sp.argsort (-eigenvalues)


    return [sp.real(eigenvalues[ind]), eigenvectors[:, ind]]
